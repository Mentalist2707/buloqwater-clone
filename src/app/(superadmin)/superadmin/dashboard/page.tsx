...
async function getDashboardData() {
  ...
  const [
    ...
    // Oylik o'sish grafigi (oxirgi 6 oy)
    getMonthlyGrowth(),
  ]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Tizimdagi umumiy ko'rsatkichlar</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          trend={data.growthPercent !== 0 ? { value: Math.abs(data.growthPercent), positive: data.growthPercent > 0 } : undefined}
          description={`Shu oy: +${data.thisMonthCompanies}`}
        />
        <StatCard
          title="Faol Kompaniyalar"
          value={data.activeCompanies}
          icon={<span className="text-xl">✅</span>}
          description={`${data.totalCompanies > 0 ? Math.round((data.activeCompanies / data.totalCompanies) * 100) : 0}% faollik`}
        />
        <StatCard
          title="Muzlatilgan"
          value={data.suspendedCompanies}
          icon={<span className="text-xl">⏸️</span>}
          description={data.suspendedCompanies > 0 ? "Diqqat talab" : "Hammasi yaxshi"}
        />
        <StatCard
          title="Jami Foydalanuvchilar"
          value={data.totalUsers}
          icon={<span className="text-xl">👥</span>}
          description="Barcha kompaniyalar bo'ylab"
        />
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500">Oxirgi 6 oy</p>
          </div>
        </div>
        <MonthlyGrowthChart data={data.monthlyData} />
      </div>

      {/* 2 ustunli layout: Top kompaniyalar + Activity Log */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Chap: Top 5 Kompaniya */}
        <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900">Eng Faol Kompaniyalar</h3>
            <Link href="/superadmin/companies" className="text-sm text-primary-500 hover:underline">
              Hammasi →
            </Link>
          </div>
          <div className="space-y-3">
            {data.topCompanies.length === 0 ? (
              <p className="text-sm text-gray-500 text-center py-4">Hali kompaniya yo'q</p>
            ) : (
              data.topCompanies.map((company, idx) => {
                const daysLeft = company.subscription
                  ? Math.ceil((new Date(company.subscription.endDate).getTime() - Date.now()) / 86400000)
                  : null;
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {company._count.orders} buyurtma · {company._count.customers} mijoz
                        </p>
                      </div>
                    </div>
        ...
        {/* O'ng: Activity Log */}
        <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Faoliyat Jurnali</h3>
                ...
                <div key={log.id} className="flex items-start gap-3 p-2 rounded-lg hover:bg-gray-50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {log.company && (
                        <span className="text-xs text-primary-500">{log.company.name}</span>
                      )}
                    </div>
                  </div>
                </div>
